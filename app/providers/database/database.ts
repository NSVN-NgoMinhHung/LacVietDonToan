...
export class Database {
  ...
  public constructor() {
    if (!this.isOpen) {
      ...
      this.storage.openDatabase({ name: 'data.db', location: 'default' }).then(() => {
        this.storage.executeSql('CREATE TABLE IF NOT EXISTS batmon (id INTEGER PRIMARY KEY, name TEXT, details TEXT)', []);
        ...
        this.storage.executeSql('CREATE TABLE IF NOT EXISTS lucnham (id INTEGER PRIMARY KEY, name TEXT, details TEXT)', []);
        ...
        this.storage.executeSql('CREATE TABLE IF NOT EXISTS tongket (idBatMon INTEGER, idLucNham INTEGER, details TEXT, PRIMARY KEY (idBatMon, idLucNham))', []);
  ...
  public getLucNham(id: number) {
    return new Promise((resolve, reject) => {
      this.storage.executeSql('SELECT * FROM lucnham where id = ?', [id]).then((data) => {
        let lucnham = [];
        if (data.rows.length > 0) {
          for (let i = 0; i < data.rows.length; i++) {
            lucnham.push(
  ...
  public getTongKet(idBatMon: number, idLucNham: number) {
    return new Promise((resolve, reject) => {
      this.storage.executeSql('SELECT * FROM tongket where idBatMon = ? AND idLucNham = ?', [idBatMon, idLucNham]).then((data) => {
        let tongket = [];
        if (data.rows.length > 0) {
          for (let i = 0; i < data.rows.length; i++) {
            tongket.push({
              idBatMon: data.rows.item(i).idBatMon,
              idLucNham: data.rows.item(i).idLucNham,
              details: data.rows.item(i).details
            });
